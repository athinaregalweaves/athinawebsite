...
import { Plus, Pencil, Trash2, Search, ChevronLeft, ChevronRight, Clock, CheckCircle2, Sparkles } from "lucide-react";
...
const AdminTissue = () => {
  ...
  useEffect(() => {
    verifyToken()
      .then(async () => {
        const tissueIds = new Set((await fetchProductIdsByCollection("tissue")).map(String));
        const allProducts = await getAllProducts();
        const tissue = allProducts.filter((p) => tissueIds.has(String(p.id)));
        setProducts(tissue.map(p => ({ ...p })));
        setLoading(false);
      })
  ...
  const handleRemove = async (id: string) => {
    ...
    await apiRemoveProductFromCollection(id, "tissue");
    setProducts(prev => prev.filter(p => p.id !== id));
    addHistoryEntry(`Removed "${name}" from Tissue`);
    addHistory({ page: 'tissue', action: `Removed "${name}"`, item_name: name, image_url: product?.image });
    toast({ title: "Removed from Tissue Collection" });
  };
      ...
      <div className="px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <AdminBuiltinCollectionTitle
              collectionKey="tissue"
              fallbackTitle="Tissue & Organza Sarees"
              icon={Sparkles}
              subtitle={`${products.length} tissue sarees`}
            />
          </div>
          <div className="flex items-center gap-3">
            ...
            <Button onClick={() => navigate("/admin/product/new?from=tissue")} className="bg-maroon hover:bg-maroon-light text-ivory font-body text-sm h-11 px-6">
              <Plus size={16} className="mr-2" /> Add Tissue Saree
            </Button>
          </div>
        </div>
        ...
        {products.length === 0 ? (
          <div className="text-center py-16 bg-background border border-border">
            <Sparkles size={48} className="mx-auto text-foreground/20 mb-4" />
            <p className="font-body text-lg text-foreground/50 mb-2">No tissue sarees yet</p>
          ...
          <>
            <div className="relative mb-6">
              ...
              <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search tissue sarees..."
                      ...
                      <tr key={product.id} className="border-b border-border/50 hover:bg-ivory-warm/50">
                        ...
                        <td className="py-3 px-4 text-center">
                          <div className="flex justify-center gap-2">
                            <Button size="sm" variant="ghost" onClick={() => navigate(`/admin/product/${product.id}?from=tissue`)}
